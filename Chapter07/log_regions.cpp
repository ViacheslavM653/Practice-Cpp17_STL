...
public:
    explicit redirect_cout_region(const string &filename)
        : ofs{filename} , buf_beckup{cout.rdbuf(ofs.rdbuf())}
    {}
    redirect_cout_region()
        :ofs{}, buf_beckup(cout.rdbuf(ofs.rdbuf()))
    {}
    ~redirect_cout_region(){
        cout.rdbuf(buf_beckup);
    }   
};
...

{
    ...
    cout << "Readeble from normal stout again\n";
